# vpsbox

VPS 初始化、系统优化与 sing-box 节点管理脚本。

当前版本：`v1.0.48`

## VPS 一键安装

```bash
bash <(curl -fsSL https://raw.githubusercontent.com/TianPingXi/vpsbox/main/vpsbox.sh)
```

## 管理命令

安装完成后，可随时输入以下命令打开管理面板：

```bash
vpsbox
```

## 主界面

输入 `vpsbox` 打开管理面板：

```text
========================================
 vpsbox
========================================
 版本：v1.0.48
 提示：输入 vpsbox 打开管理面板
----------------------------------------
 sing-box：1.x.x 运行中 / 1.x.x 未运行 / 未安装
 SSH 端口：22 / 自定义端口 / 无法读取
----------------------------------------
 VLESS Reality 节点
 状态：已创建
 名称：vless-example
 地址：vless.example.com
 端口：49880
----------------------------------------
 Shadowsocks 节点
 状态：已创建
 名称：ss-example
 地址：ss.example.com
 端口：49881
----------------------------------------
 IPv4 DNS：
 nameserver 1.1.1.1
 nameserver 8.8.8.8
----------------------------------------
 [1] 节点管理
 [2] sing-box 管理
 [3] 系统优化
 [4] 主机防火墙
 [5] 一键检测
 [6] 第三方脚本
----------------------------------------
 [00] 更新 vpsbox
 [88] 卸载 vpsbox
 [0] 退出
========================================
请输入选项:
```

## 子菜单

```text
节点管理
[1] 创建/重建 VLESS Reality 节点（推荐）
[2] 创建/重建 Shadowsocks 节点
[3] 查看节点链接
[4] 删除 VLESS Reality 节点
[5] 删除 Shadowsocks 节点
----------------------------------------
[0] 返回主菜单

sing-box 管理
[1] 启动服务
[2] 停止服务
[3] 重启服务
[4] 更新 sing-box
----------------------------------------
[0] 返回主菜单

系统优化
基础
[1] 系统更新
[2] 垃圾清理
[3] 修改主机名
[4] 开启 NTP 时间同步

网络
[5] 修改 IPv4 DNS
[6] 开启 IPv4 优先
[7] 开启 BBR + fq
[8] 禁用 IPv6
[9] TCP 缓冲区调优

SSH 安全
[10] 修改 SSH 端口
[11] SSH 基础加固
[12] 查看 SSH 生效配置
[13] 安装 Fail2ban

维护
[14] 限制 journald 日志大小
[15] 查看/恢复 vpsbox 系统改动
----------------------------------------
[0] 返回主菜单

修改 SSH 端口
当前 SSH 端口：22 / 自定义端口 / 无法读取
新端口：创建时输入，留空默认 23333
[1] 应用 SSH 端口修改
[2] 恢复 SSH 端口为 22
----------------------------------------
[0] 返回系统优化

vpsbox 系统改动
[1] 恢复全部已记录项目
[2] IPv4 DNS：无记录
[3] BBR + fq：无记录
[4] IPv4 优先：无记录
[5] Fail2ban：无记录
[6] journald 日志限制：无记录
[7] NTP 时间同步：无记录
[8] IPv6 禁用：无记录
[9] TCP 缓冲区：无记录
----------------------------------------
[0] 返回系统优化菜单

TCP 缓冲区调优
[1] 第一档（100–300 Mbps / 最大 8 MiB）
[2] 第二档（301–600 Mbps / 最大 16 MiB）
[3] 第三档（601–1000 Mbps / 最大 32 MiB）
----------------------------------------
[0] 返回系统优化菜单

主机防火墙
[1] 一键开启/更新防火墙
[2] 查看当前放行端口
[3] 管理额外放行端口
[4] 关闭并移除 vpsbox 防火墙
----------------------------------------
[0] 返回主菜单

额外放行端口
[1] 添加 TCP 端口
[2] 添加 UDP 端口
[3] 同时添加 TCP/UDP 端口
[4] 删除 TCP 端口
[5] 删除 UDP 端口
[6] 清空额外端口
----------------------------------------
[0] 返回主机防火墙

第三方脚本
[1] IP 质量体检脚本（xykt）
[2] 网络质量体检脚本（xykt）
[3] TCP 质量检测脚本（ibsgss）
[4] VPS 综合质量测试脚本（LloydAsp）
[5] 一键 VPS 系统重装脚本（bin456789）
----------------------------------------
[0] 返回主菜单
```

创建或重建 VLESS Reality 节点时，Reality 目标留空会按顺序探测内置候选域名，并选择 TCP 连接与 TLS 1.3 握手耗时最低且实际协商到 H2 的目标。DNS 解析只作为可用性前提，不计入延迟；探测不会请求 HTTP 页面。手动输入域名时仍只检查指定目标。
